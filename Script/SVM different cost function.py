...
print(f"{'C Value':<10} | {'Mean Accuracy':<15}")

for c in C_values:
    c_results = []
    for r in range(n_repeats):
        # Splitting data each time to randomize the test: 80% Training, 20% Testing
        ...
        c_results.append(acc)
    #Computing the mean accuracy 
    mean_acc = np.mean(c_results)
    print(f"{c:<10} | {mean_acc:<15.2%}")
